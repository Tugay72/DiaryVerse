    ...
    db.get("SELECT * FROM diaries WHERE user_id = ? AND date = ?", [userId, formattedDate], (err, row) => {
        ...
        db.run(
            ...
            function (err) {
                ...
                db.get("SELECT current_streak FROM users WHERE user_id = ?", [userId], (err, user) => {
                    if (err || !user) {
                    ...
                    db.get(
                        ...
                        (err, prevDiary) => {
                            ...
                            const newStreak = prevDiary ? user.current_streak + 1 : 1;

                            // Update the streak in the users table
                            db.run(
